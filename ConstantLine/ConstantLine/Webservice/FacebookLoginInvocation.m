...
@implementation FacebookLoginInvocation

@synthesize email,displayName,UserImage,gender,dob,rerg,fbId;
...
-(NSString*)body {
	...
	[bodyD setObject:self.fbId forKey:@"fbId"];
    [bodyD setObject:self.displayName forKey:@"display_name"];
    [bodyD setObject:self.dob forKey:@"dob"];
 	[bodyD setObject:self.gender forKey:@"gender"];
 	[bodyD setObject:self.UserImage forKey:@"image"];
    ...
    if ([resultsd count]>0) {
        ...
        if ([self.rerg objectForKey:@"error"]==nil || [self.rerg objectForKey:@"error"]==(NSString*)[NSNull null]) {
            ...
            if (gUserImage==nil || gUserImage==(NSString*)[NSNull null] || [gUserImage isEqualToString:@""]) {
                ...
                NSString *userImage=[NSString stringWithFormat:@"%@",gUserImage];
        ...
        [self.delegate FacebookLoginInvocationDidFinish:self withResults:[self.rerg objectForKey:@"success"] withMessages:[self.rerg objectForKey:@"error"] withError:error];
...
-(BOOL)handleHttpError:(NSInteger)code {
	[self.delegate FacebookLoginInvocationDidFinish:self
                                withResults:Nil
                               withMessages:Nil
