...
- (void)displayImage {
		...
		if (index != NSNotFound) {
			...
			UIImage* image =  [self.photoBrowser imageAtIndex:index];
			UIImage *img;
			
			if (image.size.height<=400) {
				
				img =[image resizedImage:CGSizeMake(310.0f, image.size.height) interpolationQuality:1];
				//[_secondImg setFrame:CGRectMake(_secondImg.frame.origin.x,_secondImg.frame.origin.y,_secondImg.frame.size.width,img.size.height)];
				
			}
			else {
				
				img =[image resizedImage:CGSizeMake(310.0f, 400.0f) interpolationQuality:1];
				//[_secondImg setFrame:CGRectMake(_secondImg.frame.origin.x,_secondImg.frame.origin.y,_secondImg.frame.size.width,250)];
				
			}
			
			
			//UIImage *img =[image resizedImage:CGSizeMake(310.0f, 400.0f) interpolationQuality:1];
			...
			if (img) {
				...
				[photoImageView setFrame:CGRectMake(photoImageView.frame.origin.x, photoImageView.frame.origin.y, 310, 400)];
				photoImageView.image=[UIImage imageNamed:@"PholderBrowser.png"];
